...
const { Client, AuthError } = SimpleHMACAuth;
...
describe('Client class', () => {
  ...
  test('respects options', () => {
    ...
    expect(client._settings.options).toEqual({ test: true });
  ...
  test('outputs warnings when instantiated with an empty secret', () => {
  ...
  test('resolves requests with callbacks', (done) => {
    ...
    client.call('GET', '/v1/items/', null, null, (error, response) => {
      expect(error).toBeUndefined();
      expect(response).toBe('OK');
  ...
  test('rejects requests after specified timeout', async () => {
    ...
    try {
      ...
      expect(timedOutAfter < (timeout + 50)).toBe(true);
    }
